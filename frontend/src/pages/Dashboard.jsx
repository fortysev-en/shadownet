...
const Dashboard = () => {
	...
	const { termID } = useSelector((response) => response.TerminateHost);
	const { user } = useSelector((response) => response.AuthUser);
	const { dashboardData } = useSelector((response) => response.GetDashboard);
	...
	return (
		<Container className="mt-10">
			...
			<div className="w-full flex flex-col gap-5">
				<div className="h-60 w-full rounded-xl bg-muted/50 p-5 flex flex-col gap-8">
					<div className="w-full flex items-center justify-between px-2">
						<div className="flex flex-col">
							<h3 className=" font-semibold">
								{user.first_name}'s Connection History
							</h3>
							<h6 className="text-sm text-muted-foreground">
								Graph displays number of connected devices
							</h6>
						</div>
					</div>
					...
					<div className="w-full h-[55vh] rounded-xl bg-muted/50 flex flex-col gap-8 p-5">
						<div className="w-full flex items-center justify-between px-2">
							...
							<h3 className=" font-semibold">
								{dashboardData &&
									dashboardData.hosts &&
									dashboardData.hosts.length}
							</h3>
						</div>
						<div className="flex flex-col gap-2 h-full overflow-scroll scrollbar-hide">
							{dashboardData &&
								dashboardData.hosts &&
								dashboardData.hosts.map((host, id) => (
									<div
										onClick={() =>
											navigate(
												`/shadownet/host?id=${host.identifier}&hostname=${host.hostname}`
											)
										}
										className={
											termID && termID === host.identifier
												? "w-full flex items-center gap-4 p-2 rounded-xl pointer-events-none select-none opacity-60 cursor-default"
												: "w-full flex items-center gap-4 p-2 rounded-xl hover:bg-muted-foreground/10 cursor-default"
										}
										key={id}
									>
										<div className="w-11 h-11 flex items-center justify-center rounded bg-muted p-2">
											{host.os === "Windows" ? (
												<svg
												...
												>
													<path
														d="M4 4H14V14H4zM16 4H26V14H16zM4 16H14V26H4zM16 16H26V26H16z"
										...
										<div className="flex items-center justify-between w-full">
											...
											<div className="flex flex-col items-end">
												<h3 className=" font-semibold">#PN {host.port}</h3>
												<h6 className="text-sm text-muted-foreground truncate text-ellipsis">
													{host.ip}
												</h6>
											</div>
										</div>
